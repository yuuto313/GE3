...
void ParticleCommon::PreDraw()
{
	...
	pDxCommon_->GetCommandList()->SetGraphicsRootSignature(rootSignature_.Get());
	// PSOを設定
	pDxCommon_->GetCommandList()->SetPipelineState(graphicsPipelineState_.Get());
	// 形状を設定。PSOに設定してるものとはまた別。同じものを設定すると考えておけばいい
	pDxCommon_->GetCommandList()->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
}
...
void ParticleCommon::CreateRootSignature()
{
	...
	rootParameters_[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE; //CBVを使う
	rootParameters_[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX; //VertexShaderで使う
	rootParameters_[1].DescriptorTable.pDescriptorRanges = descriptorRange_;// Tableの中身の配列を指定
	...
	//バイナリを元に作成
	hr = pDxCommon_->GetDevice()->CreateRootSignature(0, signatureBlob->GetBufferPointer(), signatureBlob->GetBufferSize(), IID_PPV_ARGS(&rootSignature_));
	assert(SUCCEEDED(hr));
	...
	// Shaderをコンパイラする
	vertexShaderBlob_ = pDxCommon_->CompileShader(L"Resources/shaders/Particle.VS.hlsl", L"vs_6_0");
	assert(vertexShaderBlob_ != nullptr);

	pixelShaderBlob_ = pDxCommon_->CompileShader(L"Resources/shaders/Particle.PS.hlsl", L"ps_6_0");
...
void ParticleCommon::CreateGraphicsPipeline()
{
	...
	// 実際に生成
	HRESULT hr = pDxCommon_->GetDevice()->CreateGraphicsPipelineState(&graphicsPipelineStateDesc_, IID_PPV_ARGS(&graphicsPipelineState_));
	assert(SUCCEEDED(hr));
}

Microsoft::WRL::ComPtr<ID3D12PipelineState> ParticleCommon::CreateGraphicsPipelineState(Microsoft::WRL::ComPtr<ID3D12Device> device, const D3D12_GRAPHICS_PIPELINE_STATE_DESC& desc)
{
